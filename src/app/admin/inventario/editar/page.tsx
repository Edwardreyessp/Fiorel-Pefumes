...
import { TagPicker } from '../crear/TagPicker';

const CreatePage = (): JSX.Element => {
	const { updatePerfume, selectedPerfume } =
		useContext<PerfumeContextProps>(PerfumeContext);
	const { selectedTags, setErrors, updateAlert, updateTags } =
	...
	useEffect(() => {
		setNewPerfume(selectedPerfume);
		form.setValue('name', selectedPerfume.name);
		form.setValue('brand', selectedPerfume.brand);
		form.setValue('description', selectedPerfume.description);
		form.setValue('price', selectedPerfume.price.toString());
		console.log({
			categoryID: selectedPerfume.category,
			familyID: selectedPerfume.family,
			essencesID: selectedPerfume.essences,
		});
		updateTags({
			categoryID: selectedPerfume.category,
			familyID: selectedPerfume.family,
	...
	return (
		<Stack spacing={2}>
			...
			>
				...
				<Stack spacing={2} flex={{ xs: 1, md: 4 / 3 }} width='100%'>
					<Button
						variant='contained'
						fullWidth
						onClick={() => {
					...
					>
						{selectedPerfume.status === 'draft'
							? 'Guardar'
							: 'Guardar y quitar publicación'}
					</Button>
				</Stack>
			</Box>
		</Stack>
