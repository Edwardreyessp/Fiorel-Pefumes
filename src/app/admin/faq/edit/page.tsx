...
import { AccordionFAQ } from "@/app/components/atoms";
...
export default function EditsFAQsPage(): JSX.Element {
  ...
  return <main><Stack spacing={2} >
    ...
    <Box display={{ sx: "", md: "flex" }} justifyContent={"space-around"} width={"100%"} sx={{ borderBottom: `1px solid #000` }} pb={3}>
    ...
    {fileData?.slice(1).map((faq, index) => {
      return <AccordionFAQ
        key={index}
        title={faq.Pregunta?.toString() ?? ""}
        description={faq.Respuesta?.toString() ?? ""}
      />
    })}
